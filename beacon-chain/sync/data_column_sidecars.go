...
// FetchDataColumnSidecars retrieves data column sidecars from storage for the given map
// of block roots to their requested data column indices. The function employs a two-step
...
//
// Returns a map of block roots to their corresponding verified read-only data columns,
// and a map of block roots to indices of unavailable sidecars that could not be retrieved
// or reconstructed. Returns an error only if data column storage is unavailable or if
// storage/reconstruction operations fail unexpectedly.
...
func tryRequestingColumnsFromPeers(
	...
	indicesByRootByPeer, err := explorePeers(p.P2P, slotByRoot, missingIndicesByRoot, connectedPeers)
	if err != nil {
	...
	for len(missingIndicesByRoot) > 0 && len(indicesByRootByPeer) > 0 {
		...
		batchVerifiedRoDataColumnSidecars, err := verifyDataColumnsSidecarsByPeer(p.P2P, p.NewVerifier, roDataColumnsByPeer)
		if err != nil {
...
// verifyDataColumnsSidecarsByPeer verifies the received data column sidecars.
...
func verifyDataColumnsSidecarsByPeer(
	p2p prysmP2P.P2P,
	newVerifier verification.NewDataColumnsVerifier,
...
// explorePeers returns a peers->root->indices map.
func explorePeers(
	p2p prysmP2P.P2P,
	...
	// TODO: Avoid this loop in loop in loop
	peersByIndexByRoot := make(map[[fieldparams.RootLength]byte]map[uint64]map[goPeer.ID]bool)
	...
	for blockRoot, indices := range indicesByBlockRoot {
		...
		for index := range indices {
			...
			for peer := range peers {
				...
				// Build root->index->peers map
				if _, exists := peersByIndexByRoot[blockRoot]; !exists {
					peersByIndexByRoot[blockRoot] = make(map[uint64]map[goPeer.ID]bool)
				}
				if _, exists := peersByIndexByRoot[blockRoot][index]; !exists {
					peersByIndexByRoot[blockRoot][index] = make(map[goPeer.ID]bool)
				}
				peersByIndexByRoot[blockRoot][index][peer] = true
...
// randomPeer selects a random peer from the given nested map.
...
func randomPeer(
	...
	for ctx.Err() == nil {
		...
		for peer := range indicesByRootByPeer {
			if rateLimiter.Remaining(peer.String()) >= int64(count) {
